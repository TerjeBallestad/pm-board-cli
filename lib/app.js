...
export async function createApp({ frontend = true, attachments = true } = {}) {
  ...
  // A long-running server keeps its boot-time code in RAM; if npm swaps the
  // package underneath it (manual npm install without a restart), it silently
  // serves stale code — the failure mode behind issue #1's original suspicion.
  // Compare boot VERSION to package.json on disk and flag the skew.
  const pkgPath = join(__dirname, '..', 'package.json');
  const installedVersion = () => {
    try { return JSON.parse(readFileSync(pkgPath, 'utf-8')).version; } catch { return VERSION; }
  };
  let warnedStale = false;
  app.get('/api/health', (req, res) => {
    const installed = installedVersion();
    const staleCode = installed !== VERSION;
    if (staleCode && !warnedStale) {
      warnedStale = true;
      console.error(`Warning: server is running v${VERSION} but v${installed} is installed — restart with \`pm serve --restart\``);
    }
    res.json({
      status: 'ok',
      version: VERSION,
      ...(staleCode ? { staleCode: true, installedVersion: installed } : {}),
      ...(store.getRewind() ? { rewound: store.getRewind() } : {}),
      project: config.name
    });
  });
